...
\begin{abstract}
    ...
    以往的研究主要基于的IVy进行实现，\TLA 语言领域的相关研究较少。
    本文将提供一种基于\TLA 的归纳不变式生成工具\rltla ，实现对以\TLA 语言描述的分布式协议规约的归纳不变式的自动化生成。
    利用强化学习的方法，加速归纳不变式的推到过程。
    通过引入TLC或Apalache 做为模型检查器，实现对归纳不变式的验证。
    通过实验，验证了\rltla 的有效性，证明了强化学习在归纳不变式生成中应用的可行性。
...
\begin{abstract*}
    ...
    This thesis will provide an inductive invariant generation tool based on \TLA, called \rltla , which realizes the automatic generation of inductive invariants for distributed protocol specifications described in \TLA language.
    It accelerates the process of inductive invariant derivation using reinforcement learning methods,
    and verification of inductive invariants is implemented well by introducing TLC or Apalache, model checkers.
    Through experiments, the effectiveness of \rltla , and the feasibility of applying reinforcement learning in inductive invariant generation have been proved.
\end{abstract*}
