...
\begin{document}
\tlatex
\@x{}\moduleLeftDash\@xx{ {\MODULE} Client\_Server}\moduleRightDash\@xx{}%
\@pvspace{8.0pt}%
\@x{ {\EXTENDS} Server ,\, Client}%
\@x{ {\VARIABLE} locked ,\, held}%
\@pvspace{8.0pt}%
\@x{ Init \.{\defeq}}%
 \@x{\@s{16.4} \.{\land} locked \.{=} [ i \.{\in} Server \.{\mapsto} {\TRUE}
 ]}%
\@x{\@s{16.4} \.{\land} held \.{=} [ i \.{\in} Client \.{\mapsto} \{ \} ]}%
\@pvspace{8.0pt}%
\@x{ Connect ( client ,\, server ) \.{\defeq}}%
\@x{\@s{16.4} \.{\land}\@s{8.95} locked [ server ] \.{=} {\TRUE}}%
 \@x{\@s{16.4} \.{\land}\@s{8.95} held \.{'} \.{=} [ held {\EXCEPT} {\bang} [
 client ] \.{=} held [ client ] \.{\cup} \{ server \} ]}%
 \@x{\@s{16.4} \.{\land}\@s{8.95} locked \.{'} \.{=} [ locked {\EXCEPT}
 {\bang} [ server ] \.{=} {\FALSE} ]}%
\@pvspace{8.0pt}%
\@x{ Disconnect ( client ,\, server ) \.{\defeq}}%
\@x{\@s{16.4} \.{\land} server \.{\in} held [ client ]}%
 \@x{\@s{16.4} \.{\land} held \.{'} \.{=} [ held {\EXCEPT} {\bang} [ client ]
 \.{=} held [ client ] \.{\,\backslash\,} \{ server \} ]}%
 \@x{\@s{16.4} \.{\land} locked \.{'} \.{=} [ locked {\EXCEPT} {\bang} [
 server ] \.{=} {\TRUE} ]}%
\@pvspace{8.0pt}%
\@x{ Next \.{\defeq}}%
 \@x{\@s{16.4} \.{\lor} \E\, client \.{\in} Client ,\, server \.{\in} Server
 \.{:} Connect ( client ,\, server )}%
 \@x{\@s{16.4} \.{\lor} \E\, client \.{\in} Client ,\, server \.{\in} Server
 \.{:} Disconnect ( client ,\, server )}%
\@pvspace{8.0pt}%
 \@x{ Spec \.{\defeq} Init \.{\land} {\Box} [ Next ]_{ {\langle} locked ,\,
 held {\rangle}}}%
\@pvspace{8.0pt}%
\@x{ Safe\@s{0.32} \.{\defeq}}%
\@x{\@s{16.4} \A\, client\_ \.{<} i \.{>} ,\, client\_j \.{\in} Client \.{:}}%
 \@x{\@s{16.4} ( held [ client\_i ] \.{\cap} held [ client\_j ] \.{\neq} \{ \}
 ) \.{\lor} ( client\_i \.{=} client\_j )}%
\@pvspace{8.0pt}%
\@x{}\bottombar\@xx{}%
\end{document}
